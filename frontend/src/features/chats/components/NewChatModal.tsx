...
export default function NewChatModal({
    ...
    onClose: () => void;
    onChatCreated: (chat: ChatItem | string) => void;
}) {
                ...
                <div className="overflow-y-auto p-2">
                    {connectionsQuery.isPending ? (
                        ...
                        connectionsQuery.data.map(({ friend }) => (
                            <div
                                key={friend.id}
                                className="flex items-center gap-3 rounded-xl p-3 hover:bg-surface-elevated"
                            >
                                {friend.photo ? (
                                    <img
                                        src={friend.photo}
                                    ...
                                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-brand-500/10 font-semibold text-brand-500">
                                        {(friend.name ?? "?")
                                            .charAt(0)
                                            .toUpperCase()}
                                    </div>
                                ...
                                <button
                                    ...
                                    className="flex shrink-0 items-center gap-1.5 rounded-lg bg-brand-600 px-3 py-2 text-xs font-semibold text-white hover:bg-brand-500 disabled:opacity-60"
                                >
